...
import com.tylx.leasephone.databinding.FragmetPhoneLeaseBinding;
...
import com.tylx.leasephone.util.activity.BaseRecyclerViewListFragment;
...
public class PhoneLeaseFragment extends BaseRecyclerViewListFragment2<GoodsModel, ItemHomeBinding> {
    FragmetPhoneLeaseBinding binding;

    @Override
    protected View onCreateContentView() {
        binding = DataBindingUtil.inflate(getLayoutInflater(), R.layout.fragmet_phone_lease, _containerLayout, false);
    ...
    protected void getListData() {
        super.getListData();
        new MemberModel().leaseList(LeaseFragment.PHONE_CODE, page, new BaseModel.BaseModelIB() {
            @Override
            public void StartOp() {
    ...
    public void bindItemData(final ItemHomeBinding itemHomeBinding, final GoodsModel info, int position) {
        GoodsListModel.setDatas(itemHomeBinding, info, mActivity,GoodsListModel.LEASE_PHONE_DATA);
    }
}
