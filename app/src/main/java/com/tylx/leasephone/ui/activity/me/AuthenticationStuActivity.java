 ...
 * 实名认证学生界面 传三张图的，我不知道为什么我把学生和在职分开了，看似代码都一样，但是合起来就会错
 * AuthenticationDataActivity 代替
 *
 */

public class AuthenticationStuActivity extends BaseActivity implements View.OnClickListener {
    ...
    public static void into(Activity activity, CertificatesModel model) {
        Intent intent = new Intent(activity, AuthenticationStuActivity.class);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.au_id_end_time://身份证结束时间
                if (null == mIdTimeView) {
                    mIdTimeView = new TimePickerView.Builder(AuthenticationStuActivity.this, new TimePickerView.OnTimeSelectListener() {
                        @Override
            ...
            case R.id.au_student_end_time://学生证证结束时间
                if (null == mStuTimeView) {
                    mStuTimeView = new TimePickerView.Builder(AuthenticationStuActivity.this, new TimePickerView.OnTimeSelectListener() {
                        @Override
                        public void onTimeSelect(Date date, View v) {//选中事件回调
    ...
    private void goAu() {
        studentCard = binding.auStudentNumTv.getText().toString();
        ...
        if(idCardFile == null || personalFile == null || studentFile == null){
        ...
        if (TextUtils.isEmpty(studentCardValidEnd)) {
            startEmptyAnim(binding.auStudentNumTv);
            return;
        }

        new MemberModel().uploadCertificatesInfo(DatasStore.getCurMember().id, studentCard, idCard, memberName, studentCardValidEnd, idCardValidEnd,
                "", "","","","", idCardFile, personalFile, studentFile, new BaseModel.BaseModelIB() {
                    @Override
                    public void StartOp() {
                        showLoading(true);
                    }
...
//            final ArrayList<String> images = data.getStringArrayListExtra(ImageSelectorActivity.REQUEST_OUTPUT);
    ...
    private void showImage() {
        for (int i = 0; i < imagePaths.size(); i++) {
            ...
            Glide.with(AuthenticationStuActivity.this).load(imagePaths.get(i)).into(imageView);
            final int finalI1 = i;
            delete.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (finalI1 == 2)
                        studentFile = null;
                    if(finalI1 ==0)
                        Glide.with(AuthenticationStuActivity.this).load(R.mipmap.add_camera).into(imageView);
                    else
                        Glide.with(AuthenticationStuActivity.this).load(R.mipmap.add_jia).into(imageView);
                }
            });
            ...
            new CompressImageUtil(null).compress(imagePaths.get(i), new CompressImageUtil.CompressListener() {
                ...
                public void onCompressSuccess(String imgPath) {
                    Glide.with(AuthenticationStuActivity.this).load(imgPath).into(imageView);
    ...
    public void showEdit() {
        ...
        if (type == NAME) {
            ...
            editDialog.getEditText().setHint("请输入您的学生证号码");
        }
        editDialog.show();
    }
}
